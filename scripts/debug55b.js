...
(async()=>{
  ...
  try{
    ...
    await p.evaluate(async function(){
      var V=window.video, targets=[10,30,50];
      for(var i=0;i<targets.length;i++){
        try{ V.currentTime=targets[i]; }catch(e){}
        var t0=Date.now();
        while(V.seeking&&Date.now()-t0<8000)await new Promise(r=>setTimeout(r,100));
        await new Promise(r=>setTimeout(r,400));
      }
    });
    console.log('MSE-сики прошли, t=', await p.evaluate(()=>window.video.currentTime.toFixed(2)));
    await p.evaluate(function(){
      var i=window.items.findIndex(function(it){return it.name==='long30.mp4'});
      window.openItem(i,{});
    });
    await sleep(2500);
    console.log('после openItem long30: t=', await p.evaluate(()=>window.video.currentTime.toFixed(2)), ' mseb=', await p.evaluate(()=>!!window.video._mseb));
    ...
    console.log('resume:',g1,g2,' t=',await p.evaluate(()=>window.video.currentTime.toFixed(2)));
    for(let i=0;i<10;i++){
      await sleep(2000);
