#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Мок сервера ПЛЁНКА 3.11 (v57): / → v57, /seek → стенд v51, /lab → лаба v51,
/__medialist, /__srvlog; /t/v/1 → X-Plenka-FrameT:0, /t/v/2 → 3600000.
Мост-мок 3.11: журнал mediaState/setPipAuto, __mockBgSet(true/false) —
...
HTML = os.path.join(ROOT, 'download', 'plenka-optimized-57.html')
...
BRIDGE = r"""
<script>
/* MOCK PlenkaNative 3.11 — строгая арность; журнал mediaState/setPipAuto;
...
(function(){
  ...
  window.PlenkaNative={
    ...
    version:arity0('version',function(){return 'PLENKA native 3.11 (v57, MOCK)'}),
...
if __name__ == '__main__':
    with TS(('127.0.0.1', PORT), H) as httpd:
        print('mock 3.11 on http://127.0.0.1:%d/ (v57)' % PORT, flush=True)
        httpd.serve_forever()
