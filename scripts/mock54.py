#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Мок сервера ПЛЁНКА 3.8 (v54): / и /seek → v54 + МОК-мост (строгая арность);
/t/v/1 → X-Plenka-FrameT:0 (первый); /t/v/2 → 3600000 (дальний — кейс 4K);
/t/a/3 → арт без заголовка; /v/ /a/ с Range; /list.
54: /hangnext — «заминка»: СЛЕДУЮЩИЙ Range-запрос со start≥1500000 висит 90с
(имитация мёртвой отдачи FUSE/гиганта: сик «не садится», error-события нет) —
страница обязана вылечиться сама (сик-вочдог/поток-вочдог → перезарядка).
/v/2 отдаётся медленно (64КБ / 0.5с ≈ 128КБ/с) — браузер НЕ успевает буферизовать
весь файл, дальний сик обязан выйти в сеть; после первой заминки отдача мгновенная
(проверяем именно выздоровление, а не терпение)."""
import http.server, socketserver, json, os, re, sys, threading, time

PORT=8977
ROOT='/home/z/my-project'
HTML=os.path.join(ROOT,'download','plenka-optimized-54.html')
FILES={
...
BRIDGE=r"""
<script>
/* MOCK PlenkaNative 3.8 — строгая арность (как после PiP на устройстве) */
(function(){
  var SCAN=%SCAN%;
  ...
  window.PlenkaNative={
    ...
    version:arity0('version',function(){return 'PLENKA native 3.8 (v54, MOCK)'}),
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=='/hangnext':
            HANG['armed']=True; HANG['used']=0
            self._send(200,'text/plain; charset=utf-8',b'armed'); return
        ...
        if m:
            ...
            if rng:
                ...
                if HANG['armed'] and s>=1500000 and HANG['used']<1:
                    HANG['used']+=1
                    sys.stderr.write('ock] HANG range bytes=%d-%d\n'%(s,e))
                    time.sleep(90)
                if key=='v/2' and HANG['used']<1:
                    self._stream(fp,mime,s,e,size)   # 54: медленная отдача — буфер не наедает файл
                    return
                with open(fp,'rb') as f:
                    f.seek(s); b=f.read(e-s+1)
                self._send(206,mime,b,rng='bytes %d-%d/%d'%(s,e,size)); return
            if key=='v/2' and HANG['used']<1:
                self._stream(fp,mime,0,size-1,size,fast_first=True); return
            with open(fp,'rb') as f: b=f.read()
            self._send(200,mime,b); return
        self._send(404,'text/plain; charset=utf-8',b'not found')

    def _stream(self,fp,mime,s,e,size,fast_first=False):
        """54: v/2 раздаём по 64КБ с паузами (≈128КБ/с) — Chromium не буферизует весь
        ...
        try:
            ...
            with open(fp,'rb') as f:
                f.seek(s); remaining=e-s+1; first=True
                while remaining>0:
...
if __name__=='__main__':
    with TS(('127.0.0.1',PORT),H) as httpd:
        print('mock 3.8 on http://127.0.0.1:%d/ (и /seek, /list, /hangnext)'%PORT,flush=True)
        httpd.serve_forever()
