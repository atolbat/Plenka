#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Мок сервера ПЛЁНКА 3.17 (v64): / → v64, /seek → стенд v51, /lab → лаба v51,
...
HTML = os.path.join(ROOT, 'download', 'plenka-optimized-64.html')
SEEK = os.path.join(ROOT, 'scripts', 'from_chat_plenka-seek-v51.html')
LAB = os.path.join(ROOT, 'scripts', 'from_chat_plenka-lab-v51.html')
def _opt(path):
    """64: медиа-файлы прошлых сессий могли не сохраниться — смоук v64 не играет
    видео, поэтому отсутствующие файлы просто отдают 404, а крошечный JPEG
    генерируем сами (1×1)."""
...
_TJ = os.path.join(ROOT, 'scripts', 'tiny_thumb.jpg')
if os.path.exists(_TJ):
    TINY_JPEG = open(_TJ, 'rb').read()
else:
    # 64: генерируем крошечный JPEG сами (6×4, качество 60) — файл прошлых сессий не сохранился
    import io
    try:
        from PIL import Image
        _im = Image.new('RGB', (6, 4), (232, 163, 61))
        _bf = io.BytesIO()
        _im.save(_bf, 'JPEG', quality=60)
        TINY_JPEG = _bf.getvalue()
    except Exception:
        TINY_JPEG = b''
LIST = {"ok": True, "items": [
...
(function(){
  ...
  window.PlenkaNative={
    ...
    version:arity0('version',function(){return 'PLENKA native 3.17 (v64, MOCK)'}),
    ...
    def do_GET(self):
        ...
        if m:
            ...
            if key not in FILES:
                ...
                return
            fp, mime = FILES[key]
            if not os.path.exists(fp):   # 64: медиа прошлых сессий не сохранилось — смоуку v64 не нужно
                self._send(404, 'text/plain; charset=utf-8', b'no media file', {'X-Plenka-Err': 'no-media'})
                return
...
if __name__ == '__main__':
    with TS(('127.0.0.1', PORT), H) as httpd:
        print('mock 3.17 on http://127.0.0.1:%d/ (v64)' % PORT, flush=True)
        httpd.serve_forever()
