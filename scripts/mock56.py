#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Мок сервера ПЛЁНКА 3.10 (v56): / → v56 (фон без PiP),
/seek → стенд v51, /lab → лаба v51, /__medialist, /__srvlog;
/t/v/1 → X-Plenka-FrameT:0 (первый кадр), /t/v/2 → 3600000 (дальний);
/hangnext — дальний Range висит 90с (мёртвая отдача → вочдог перезаряжает);
v/2 медленный. Мост-мок 3.10 пишет ВСЕ вызовы mediaState (с временами),
setPipAuto (со значениями) и умеет __mockBgSet(true/false) — эмуляция
document.hidden + visibilitychange для проверки фоновых веток страницы."""
import http.server, socketserver, json, os, re, sys, threading, time

PORT = 8977
ROOT = '/home/z/my-project'
HTML = os.path.join(ROOT, 'download', 'plenka-optimized-56.html')
...
BRIDGE = r"""
<script>
/* MOCK PlenkaNative 3.10 — строгая арность (как после PiP на устройстве).
   Плюс v56: журнал ВЫЗОВОВ mediaState (время+playing), setPipAuto со значениями,
...
(function(){
  ...
  window.PlenkaNative={
    ...
    version:arity0('version',function(){return 'PLENKA native 3.10 (v56, MOCK)'}),
...
if __name__ == '__main__':
    with TS(('127.0.0.1', PORT), H) as httpd:
        print('mock 3.10 on http://127.0.0.1:%d/ (v56: __mockBgSet, mediaState-журнал)' % PORT, flush=True)
        httpd.serve_forever()
