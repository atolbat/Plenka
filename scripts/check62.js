#!/usr/bin/env node
/* Смоук v62: превью-время при переключении + все фиксы v61 + регресс v59.
...
(async()=>{
  ...
  try{
    ...
    await sleep(1400);
    A(errs.length===0,'v62 поднялась без JS-ошибок',errs.length?errs[0]:'');
    A((await p.evaluate(()=>window.PlenkaNative.version())).indexOf('3.15')>=0,'мост представился 3.15');
    ...
    A(/v62/.test(diag),'ДИАГ: версия v62');
